...
#特别需要的路径
SPECIAL_FILE = ['C:\Users\kod\AppData\Local\mahjong','C:\Users\kod\Desktop','D:\svn\workspace','C:\workTools']
#置顶的文件
...
class Tools(Frame):
	...
	def createWidgets(self,findDir,findFiles):
		...
		for key in SPECIAL_FILE:
			file =  r'' + key
			Button(self, text = file.replace(r'' + os.path.dirname(file), ""),width = 10, command=lambda files=r'' + key : download(files)).grid(row = rowCount, column = colCount)
			colCount = colCount + 1
