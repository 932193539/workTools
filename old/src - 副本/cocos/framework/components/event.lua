...
-- remove 会删除,我只想清空
local file = io.open("../Client/src/__DispatchEvent.lua", "w")
...
function Event:dispatchEvent(event)
	...
	local file = io.open("../Client/src/__DispatchEvent.lua", "a+")
    if file then
        file:write(os.date("%H:%M:%S", kod.util.Time.now()).." 事件回调: "..tostring(event.name).."\n")
        file:close()
    end
